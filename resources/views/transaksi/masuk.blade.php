    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <h5 class="text-gray-500 m-5">
                <a href="">Transaksi</a> /
                <a href="{{ route('transaksi.masuk') }}">Request Barang</a>
            </h5>
                                ...
                                <tbody>
                                    @forelse ($transaksis as $transaksi)
                                        ...
                                        {{ $transaksi->created_at->format('d-m-y') }}
                                            ...
                                            <td class="px-6 py-4 text-red-500 font-bold capitalize">
                                                <div
                                            ...
                                            @if ($transaksi->barang->stock < $transaksi->jumlah_permintaan)
                                            ...
                                            <td class="px-6 py-4 text-gray-600 flex justify-center items-center">
                                                @if ($transaksi->barang->stock >= $transaksi->jumlah_permintaan)
                                                    <form class="flex gap-5" method="post"
                                                        action="{{ route('update.status.request', $transaksi->id) }}">
                                                        @csrf @method('PATCH')
                                                ...
                                                <form class="flex gap-5" method="post"
                                                    action="{{ route('delete.request.stock.barang', $transaksi->id) }}">
                                                    @csrf @method('DELETE')
                                                    <button
